...
const Quiz: React.FC = () => {
    ...
    let intervalId: NodeJS.Timeout | undefined;
		...
		useEffect(()=>{
			const startQuiz=async()=>{
				try {
					const res = await axios.post(`http://localhost:5000/api/quiz-attempts/`,{"userId":userId,"quizId":quizId},{
						headers: {
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            await axios.post('http://localhost:5000/api/quiz-attempts/submit', payload,{
							headers: {
									Authorization: `Bearer ${token}`, 
							},});
    ...
    return (
        <div>
            ...
            {questions.map((question) => (
                <Card key={question._id} variant="outlined" style={{ margin: '10px 0' }}>
                    <CardContent>
